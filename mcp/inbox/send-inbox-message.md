...
| `kind` | string | yes | `report` — something finished that is worth a human read. `question` — something you cannot decide without the owner; ask it in `body`, and their reply comes back through list_inbox_messages as a turn in this letter's thread. One of: `report`, `question`. |
