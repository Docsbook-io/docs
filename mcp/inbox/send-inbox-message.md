...
description: "WRITE A LETTER INTO THIS PROJECT'S INBOX — the owner's mailbox, the first screen of their panel."
---
...
| `kind` | string | yes | `report` — something finished that is worth a human read. `question` — something you cannot decide without the owner; needs `questions`. One of: `report`, `question`. |
...
| `questions` | string[] | no | For `kind: "question"` only — one short prompt per thing you need typed, in order. Each renders as its own answer box; the reply comes back from list_inbox_messages as that question's `answer`, `null` until the owner types one. |
