---
title: "MCP Tools"
description: "Every tool this MCP server registers, with its arguments and the exact call an MCP client puts on the wire."
---

# MCP Tools

Every tool this MCP server registers, with its arguments and the exact call an MCP client puts on the wire.
...
- [Product Help](./product-help/README.md) — The specialist — `docsbook_assistant` — the agent that answers HOW the work is done well, from the published…
- [Agent](./agent/README.md) — The direction agent — `docsbook_expert` — an expert that tells you WHAT to do and WHY, and does none of it.
