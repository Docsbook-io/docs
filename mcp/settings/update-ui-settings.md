...
| `home_hide_chrome` | boolean | no | HOME PAGE ONLY: remove the article chrome from the front page — the breadcrumb/copy-page bar, the "Updated" line, the "Was this page helpful?" bar and the previous/next links. |
| `home_landing_typography` | boolean | no | HOME PAGE ONLY: landing-page scale for the front page's sections — each h2 becomes a large section title and the paragraph under it reads as that section's lead. |
