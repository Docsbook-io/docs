...
tldr: "Docsbook publishes your documentation four ways a machine can consume it: SKILL.md files that teach an agent the method, an MCP server that gives it 271 typed tools, a document graph it can navigate by heading and link, and llms.txt for agents with no connection at all."
---
...
- [MCP Server](./mcp.md) — 271 typed tools over the Model Context Protocol: ask the one `docsbook` agent and get instructions back, read pages, commit them, read analytics, change settings, start agent runs
...
| MCP server | "What can I call, on which project?" | 271 tools behind one `docsbook` agent, an `instructions` block at connect time, structured errors that name the next move | Metered per call against the project's balance; discovery calls are free |
...
- **The tool count moves.** 271 is the number of tool names this build registers. The authoritative count is whatever `tools/list` returns for your token, which the MCP section of your admin panel reads live rather than from a written-down copy.
