...
tldr: "Docsbook publishes your documentation four ways a machine can consume it: SKILL.md files that teach an agent the method, an MCP server that gives it 169 typed tools, a document graph it can navigate by heading and link, and llms.txt for agents with no connection at all."
status: generated
version: "0.3"
---
...
- [MCP Server](./mcp.md) — 169 typed tools over the Model Context Protocol: your own token gets a small orientation-and-delegate surface, and `docsbook_agent` reads pages, commits them, reads analytics and changes settings on your behalf
...
| MCP server | "What can I call, on which project?" | 169 tools behind `docsbook_agent`, an `instructions` block at connect time, structured errors that name the next move | Metered per call against the project's balance; discovery calls are free |
...
- **The tool count moves.** 169 is the number of tool names this build registers today. The authoritative count is whatever `tools/list` returns for your token, which the MCP section of your admin panel reads live rather than from a written-down copy.
