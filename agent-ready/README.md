...
tldr: "Docsbook publishes your documentation four ways a machine can consume it: SKILL.md files that teach an agent the method, an MCP server that gives it 167 typed tools, a document graph it can navigate by heading and link, and llms.txt for agents with no connection at all."
status: generated
version: "0.2"
---
...
- [MCP Server](./mcp.md) — 167 typed tools over the Model Context Protocol: your own token gets a small orientation-and-delegate surface, and `docsbook_agent` reads pages, commits them, reads analytics and changes settings on your behalf
...
| MCP server | "What can I call, on which project?" | 167 tools behind `docsbook_agent`, an `instructions` block at connect time, structured errors that name the next move | Metered per call against the project's balance; discovery calls are free |
...
- **The MCP server tells your agent how to run the skill.** Four tools used to execute one on Docsbook's machines and hand back a run id (`run_docs_*`); they were removed on 12.09.2026, along with their one-round-trip-advice successor `docsbook_expert`, removed 2026-09-18. `docsbook_agent` runs the skill itself now — the method, the steps and the tool on each — inside the job it works, rather than handing your agent instructions to carry out.
...
- **The tool count moves.** 167 is the number of tool names this build registers today. The authoritative count is whatever `tools/list` returns for your token, which the MCP section of your admin panel reads live rather than from a written-down copy.
