...
Per-page health score 0-100 (PRO), combining dead-end exits and negative feedback so the owner of a large doc set does not have to cross-reference several reports by hand. Lower score = fix sooner. Pages people leave from after succeeding are exempt from the penalty. This answers WHAT, not what to do about it. If you have not already got the method from `docsbook_expert`, get it first: it names which readings answer this question, what to compare them against, and what would make the conclusion wrong. One call, changes nothing.
...
### POST /api/v1/tools/get_content_health
...
```bash
curl -X POST 'https://docsbook.io/api/v1/tools/get_content_health' \
  -H 'Authorization: Bearer dbk_YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{"args":{}}'
```
