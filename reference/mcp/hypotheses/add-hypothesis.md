...
### POST /api/v1/tools/add_hypothesis
...
```bash
curl -X POST 'https://docsbook.io/api/v1/tools/add_hypothesis' \
  -H 'Authorization: Bearer dbk_YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{"args":{"key":"<key>","text":"<text>","kind":"forecast","status":"untested","verdict":"confirmed"}}'
```
