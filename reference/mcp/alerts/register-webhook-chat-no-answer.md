---
title: "Register webhook chat no answer"
description: "Register a webhook for the 'chat_no_answer' event (BUSINESS) — the direct call for 'notify us when…', 'alert me if…', 'ping our channel when…' about this event."
---
...
curl -X POST 'https://docsbook.io/api/mcp/server' \
  ...
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"register_webhook_chat_no_answer","arguments":{"url":"<url>"}}}'
```
...
### POST /api/v1/tools/register_webhook_chat_no_answer
...
```bash
curl -X POST 'https://docsbook.io/api/v1/tools/register_webhook_chat_no_answer' \
  -H 'Authorization: Bearer dbk_YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{"args":{"url":"<url>"}}'
```
