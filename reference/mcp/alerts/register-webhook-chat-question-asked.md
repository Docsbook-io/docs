---
title: "Register webhook chat question asked"
description: "Register a webhook for the 'chat_question_asked' event (BUSINESS) — the direct call for 'notify us when…', 'alert me if…', 'ping our channel when…' about this event."
---
...
```json
{
  ...
  "method": "tools/call",
  "params": {
    "name": "register_webhook_chat_question_asked",
    "arguments": {
      "url": "<url>"
    }
  }
...
curl -X POST 'https://docsbook.io/api/mcp/server' \
  ...
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"register_webhook_chat_question_asked","arguments":{"url":"<url>"}}}'
```
...
### POST /api/v1/tools/register_webhook_chat_question_asked
...
```bash
curl -X POST 'https://docsbook.io/api/v1/tools/register_webhook_chat_question_asked' \
  -H 'Authorization: Bearer dbk_YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{"args":{"url":"<url>"}}'
```
