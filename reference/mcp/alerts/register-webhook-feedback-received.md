---
title: "Register webhook feedback received"
description: "Register a webhook for the 'feedback_received' event (BUSINESS) — the direct call for 'notify us when…', 'alert me if…', 'ping our channel when…' about this event."
---
...
```json
{
  ...
  "method": "tools/call",
  "params": {
    "name": "register_webhook_feedback_received",
    "arguments": {
      "url": "<url>"
    }
  }
...
curl -X POST 'https://docsbook.io/api/mcp/server' \
  ...
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"register_webhook_feedback_received","arguments":{"url":"<url>"}}}'
```
...
### POST /api/v1/tools/register_webhook_feedback_received
...
```bash
curl -X POST 'https://docsbook.io/api/v1/tools/register_webhook_feedback_received' \
  -H 'Authorization: Bearer dbk_YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{"args":{"url":"<url>"}}'
```
