...
```json
{
  ...
  "method": "tools/call",
  "params": {
    "name": "register_webhook_translation_completed",
    "arguments": {
      "url": "<url>"
    }
  }
...
curl -X POST 'https://docsbook.io/api/mcp/server' \
  ...
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"register_webhook_translation_completed","arguments":{"url":"<url>"}}}'
```
...
### POST /api/v1/tools/register_webhook_translation_completed
...
```bash
curl -X POST 'https://docsbook.io/api/v1/tools/register_webhook_translation_completed' \
  -H 'Authorization: Bearer dbk_YOUR_API_KEY' \
  -H 'Content-Type: application/json' \
  -d '{"args":{"url":"<url>"}}'
```
