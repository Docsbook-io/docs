...
| `add_memory` | Write | Record a goal, a question you would otherwise guess the answer to, or one claim the next session would re-derive. Not for findings that expire — a measurement records itself in the call history above. One line, at most 600 characters: a row nobody rereads is a row nobody acts on. |
...
One direction names the goal it decomposes, states in numbers what reaching it would take, and lists its opportunities — one row each, with how many look for it, the reading that figure came from, what your docs have against it today and who holds it now. Claims are then drawn from that list rather than from whichever reading came up first, so a confirmed change reports the demand it took.

🔴 **A direction and the changes under it are judged separately, and they are allowed to disagree.** `edit_hypothesis` says whether one change did what it predicted; `edit_direction` says whether the goal moved. The sentence this exists to make sayable is the awkward one: every claim confirmed, and the target still short — which means the rows addressed a smaller share of the demand than anybody thought.

| Tool | Billing | Description |
|---|---|---|
| `list_opportunities` | Read | The standing goal, every direction under it with its target, and its opportunities, plus `progress`: how much demand is listed, how much of it anything is even pointed at, and how much has been won. Read it before writing a claim. Named `list_audits` until 2026-09-14. |
| `add_direction` | Write | Open one. `goal_key` is optional — leave it out and it decomposes the standing goal, which is the ordinary case; for an assistant, `target` and `target_metric` are required. A direction with no target cannot come out short, and coming out short is the finding. Named `add_audit` until 2026-09-14. |
| `add_opportunity` | Write | Add one to a direction: `intent` in the words somebody types, `demand_value` with the `demand_source` it came from, `current` for what you have today, `competitor` for who holds it, and `action` from a closed list — create, rewrite, expand, structure, authority or none. Named `add_audit_finding` until 2026-09-14. |
| `edit_direction` | Write | Judge it with the target reading, move its review date, or correct it. `reached` means the goal moved, not that the changes under it were confirmed. Named `edit_audit` until 2026-09-14. |
| `edit_opportunity` | Write | Correct a row, or drop one with the reason, so the next run reading the same demand does not re-propose it. Named `edit_audit_finding` until 2026-09-14. |
...
⚡ **Every field the owner reads is plain words; the trace goes in `evidence` or `demand_source`.** `intent`, `current`, `competitor`, `potential`, `demand_note` and `drop_reason` are the row on their screen — no call ids, tool names, file paths or jargon. An assistant writing one of those as a technical note is refused; the same sentence from the owner's own panel is warned, not refused.
