...
- Hovering any line of the `Live` activity feed now offers **Fix it** for that one action, so a reader searching for something you do not have, or rating a page unhelpful, is one click from what caused it. The assistant reads the path that reader actually walked before the event, separates "the docs do not cover it" from "they cover it and it was not found", and checks whether anyone else hit the same thing before calling it a problem — one person doing one thing is a lead, not a trend. `Live`
