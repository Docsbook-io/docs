...
- **Docsbook's own manual now lives at the address people type** — `docsbook.io` serves it directly instead of bouncing to `/docs`, so a link you paste, a canonical URL and a result in search all point at the same page, and customer sites moved onto their own subdomain to make room. `SEO`
