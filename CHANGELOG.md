...
- `Translations` now works the way `Analytics` does. Each readout carries its own **Turn on** over sample figures, and pressing it runs a guide that names what a tile counts, what it does not prove and the move it leads to. The overview's tiles and its reader map are two switches; a language's own page has three — the tiles, the commit ledger and the readers table. Until now the tab had none of this: the button existed everywhere else in the panel and had never been drawn here. `Translations`
- Saying yes on one language covers the rest, so a project publishing in twelve of them is not walked through the same three reports twelve times. `Translations`
