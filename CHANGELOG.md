...
- **Chat always opens as a small corner panel now, however you open it.** It used to only stay docked to the corner when you started it from the floating Ask Docs button — every other door (the header's Ask AI, a pending search or outline query, the admin toolbar, `?chat=1`) took over the whole screen, so the same site looked different depending on which button you clicked, and our own docs looked different from every other project's. Closing it is a plain X now too, replacing a button that used to read "Visit" even when it only dismissed the chat and went nowhere. `AI chat`
- **Click-to-edit has its own on/off switch now, separate from the chat.** Arming it used to only be possible by opening the chat and forcing it into a two-pane layout beside your doc; now a single bottom-right button turns hover-to-edit on over the page directly, with no chat involved. `Editor`
- **The admin chat's answers now render with the same formatting readers see on your site** — highlighted code blocks, styled tables, proper headings — instead of plain unstyled text. `AI chat`
