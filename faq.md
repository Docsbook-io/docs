...
Yes. Docsbook's MCP server exposes 275 tools at `https://docsbook.io/api/mcp/server`, so Claude Code, Cursor or ChatGPT can ask the `docsbook` agent what to do and be handed instructions to carry out, read your pages, search them, change settings and commit new pages back. Writing requires a token authorised with read-write scope; a read-only token is refused.
