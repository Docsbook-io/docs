...
Yes. Docsbook's MCP server exposes 315 tools at `https://docsbook.io/api/mcp/server`, so Claude Code, Cursor or ChatGPT can state a goal and be handed a plan, read your pages, search them, change settings and commit new pages back. Writing requires a token authorised with read-write scope; a read-only token is refused.
