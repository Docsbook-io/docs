...
| 4. Expose to machines | Serves `llms.txt` and an MCP server with 315 tools | Assistants and agents can read your docs, and agents can edit them |
...
- **MCP server** — 315 tools over the Model Context Protocol, so Claude Code, Cursor or ChatGPT can state a goal and get a plan, read your pages, search them, and commit changes back. See [MCP server](./agent-ready/mcp.md).
