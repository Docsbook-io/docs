...
You can, but GitBook's Git Sync is two-way, so two tools would write to one repository. Pick one place to edit; if it is GitHub, follow [Migrating from GitBook to Docsbook](./migrating-from-gitbook-to-docsbook.md).
