...
tldr: "Docsbook emits FAQPage, HowTo and speakable JSON-LD from your Markdown on every page of every project — there is no setting — plus Organization, TechArticle and BreadcrumbList. Google retired the FAQ and HowTo rich results, so the markup is a machine-parsing layer now — the answer box is won by the shape of the prose, which is what our content rules cover."
---
...
Docsbook does two things toward that. It **emits machine-readable markup** describing the questions and procedures your page already contains, on every page, with nothing to switch on. And it **writes and enforces content by a set of rules** aimed at the passage, not the page — those rules are the part that actually moves the outcome, and they are on [Content rules for answer engines](./content-rules.md).
...
Three more are added automatically, each only when the page genuinely contains the matching content:
...
AEO markup is emitted for every project on every plan, and there is no setting for it. (It was a toggle, and had an `update_aeo` MCP tool, until 14 September 2026.) The agents that apply the writing rules for you are a paid capability — see [pricing](https://docsbook.io/pricing).
...
- **Custom domains do not get AEO markup.** A workspace served on its own domain renders through a different path that emits a bare `TechArticle` only — no `BreadcrumbList`, no `FAQPage`, no `HowTo`, no `speakable`. On `*.docsbook.io` addresses the full graph is emitted. This is a known gap, not a configuration you can change.
- **Nothing in Docsbook validates the markup it emits.** There is no schema linter in the pipeline and `audit_geo` does not inspect JSON-LD. Check a page with Google's Rich Results Test or the Schema Markup Validator — [Structured answers](./structured-answers.md) explains what a failure looks like.
